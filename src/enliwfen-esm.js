...
class DOMQuery {
    static selectFirst(selectors, scope = null) {
...
class Endpoint {
                        ...
                        while (currentUpdate) {
                            ...
                            if (endOfUpdate === -1) {
                                ...
                                if (currentUpdate.startsWith(jsonIdentifier)) {
                                    try {
                                        const jsonString = currentUpdate.substring(jsonIdentifier.length), jsonUpdate = JSON.parse(jsonString);
     ...
     */
    async call() {
        const { element, url, method, headers, timeout } = this.node, requestOptions = { method: method };
        if (url !== undefined) {
            if (headers) {
...
class CheckboxGroup extends Feature {
    ...
    domUpdate() {
        ...
        const { element, checkboxGroup } = this.node, form = element.form, selector = `input[type='checkbox'][name='${checkboxGroup}']`;
        let checkboxes;
        if (form) {
            /* The element of this feature is part of a form.
               Consider each matching checkbox of the form */
            checkboxes = form.querySelectorAll(selector);
        }
        else {
...
class Dialog extends Feature {
    constructor(element) {
        const dismissButtons = element.querySelectorAll("button[data-enliwfen-dismissdialog]");
        super(element);
...
class Component extends ServerInteractionFeature {
    constructor(element, domAgent) {
        super(element, domAgent);
        this.domAgent = domAgent;
        const node = this.node, interval = node.interval;
        if (node.deferred) {
    ...
    constructor(element, domAgent) {
        ...
        if (this.node.event !== null) {
            /* An event is defined to listen for.
               ...
               the specified id. */
            const datalistId = `datalist#${element.getAttribute("list")}`, datalistElement = document.querySelector(`datalist#${datalistId}`);
    ...
    handleEvent(event) {
        ...
        if ((datalist !== undefined) && (event.type == this.node.event)) {
            ...
            if (dataset.enliwfenPattern !== undefined) {
                const matches = element.value.match(dataset.enliwfenPattern);
...
class DOMAgent {
    ...
    static openDialog(element) {
        ...
        const errorContainer = document.createElement("div"), shadowRoot = errorContainer.attachShadow({ mode: "open" }), errorDialog = document.createElement("dialog"), closeButton = document.createElement("button"), body = document.body;
    ...
    mergeHtml(htmlString, domTarget) {
        ...
        const update_document = this.parser.parseFromString(htmlString, "text/html"), updates = [...update_document.body.children];
    ...
    showError(htmlString, domTarget) {
...
class FeatureFactory {
    ...
    createFeatureFromDataset(element) {
